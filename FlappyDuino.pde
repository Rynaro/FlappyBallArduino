...
int game_status;
int n_pillars = 4;
float move_speed = 3.25;
String serial_receptor;
float[] serial_ary;
...
void setup() {
  ...
  game_status = 0;
  my_duino = new Serial(this, "/dev/ttyACM0", 9600);
  my_duino.bufferUntil('\n');
}

void draw() {
  background(#2980b9);
  if (boolean(my_duino.available())) {
    serialListener();
  }
  switch(game_status) {
  case 0:
...
void serialListener() {
  serial_receptor = my_duino.readStringUntil('\n');
  if (serial_receptor != null) {
    serial_ary = float(split(serial_receptor, '/'));
    move_speed = serial_ary[0];
    if (serial_ary[1] == 0) {
      gameController();
    }
  }
